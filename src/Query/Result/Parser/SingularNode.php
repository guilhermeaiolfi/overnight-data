...
final class SingularNode extends AbstractNode
{
	...
	public function __construct(
		...
		$expectedParentFieldCount = $this->role === null
			? count($this->childFields)
			: count($this->childFields) + 1;

		if (count($this->parentFields) !== $expectedParentFieldCount) {
	...
	{
		if ($this->parent === null) {
			throw new ParserException('Unable to register a singular child record because the parent node is missing.');
		}
		...
		$criteria = $this->orderedFieldValues($this->childFields, $data);

		if ($this->role !== null) {
			$criteria = [$data[self::DISCRIMINATOR_FIELD] ?? $this->role, ...$criteria];
			$data[self::DISCRIMINATOR_FIELD] = $criteria[0];
		}

		$this->parent->mount(
			$this->container ?? throw new ParserException('Unable to mount a singular child because its container name is undefined.'),
