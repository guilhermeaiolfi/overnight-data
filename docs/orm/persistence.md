...
Graph adoption classifies each untracked representation by primary key. If all primary-key fields are readable through the binding and non-null, the representation is adopted as a clean existing record for that key. If the key is incomplete or missing, it is adopted as a new record. Full objects adopted through `sync()` use their current field values as the clean baseline for existing keyed rows, so later mutations are detected normally. If a new row uses an application-assigned key, use explicit `trackNew()` plus `adopt()` instead of relying on implicit graph adoption.
