...
- Untracked objects adopted by `sync()` become new records by default. Mark `Session::update($object)` (with a complete readable primary key through the schema) to adopt them as clean existing records, or use `identify()` for key-only existing references.
