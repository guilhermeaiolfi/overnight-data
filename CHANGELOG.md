...
- **Insert generated-value recovery** — `CycleCommandExecutor` recovers pending `DatabaseGenerator` insert fields via Cycle `ReturningInterface` (`RETURNING` + statement `rowCount()`) when available; otherwise a single DB-generated PK via `query()` + `lastInsertID($sequence)` (sequence pass-through is best-effort — Cycle’s stock driver currently ignores the name). SQLite affected rows prefer `SELECT CHANGES()` after the insert statement is closed. Docs: [`docs/orm/persistence.md`](docs/orm/persistence.md).
