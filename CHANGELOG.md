...
- **Adoption simplify** — engine entry is `attach()` (Session keeps only `adopt()` for storing a ready `RepresentationState`; `RepresentationStateAdoptionTrait` removed). Removed `Session::adoptRecord()` and private `adoptGraph()`; graph sync calls `attach()` directly. `QueryRepresentationIdentityColumns` folded into `QuerySourceIdentities`. `RepresentationAdoptionContext` only carries schema, policy, and optional identities / sourceRow / intent.
