...
- **Relation result parser int/string keys** — `IndexValueEncoder` now treats canonical base-10 integer strings (`"5"`) as the same reference key as PHP ints (`5`). MySQL drivers on derived tables (e.g. FirstOfMany window queries) often return FK columns as strings while parent PKs stay ints, which previously failed relation mounts. Missing parent refs soft-fail (orphan child dropped) instead of throwing.
...
- **Insert generated-value recovery** — `CycleCommandExecutor` recovers pending `DatabaseGenerator` insert fields via Cycle `ReturningInterface` (`RETURNING` + statement `rowCount()`) when available; otherwise a single DB-generated PK via `execute()` + `lastInsertID($sequence)` immediately, before any follow-up query (PDO SQLite clears lastInsertID on the next statement). SQLite affected rows prefer a numeric `SELECT CHANGES()` row, then a recovered positive generated key. Docs: [`docs/orm/persistence.md`](docs/orm/persistence.md).
